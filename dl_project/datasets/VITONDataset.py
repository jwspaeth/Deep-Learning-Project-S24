"""
Taken directly from https://github.com/shadow2496/VITON-HD/blob/main/datasets.py.
Package not installable so must be copied.
"""
...
class VITONDataset(data.Dataset):
        ...
        for parse_id, pose_ids in [(14, [2, 5, 6, 7]), (15, [5, 2, 3, 4])]:
            ...
            for i in pose_ids[1:]:
                if (pose_data[i_prev, 0] == 0.0 and pose_data[i_prev, 1] == 0.0) or (
                    ...
                    continue
                mask_arm_draw.line(
                    [tuple(pose_data[j]) for j in [i_prev, i]], "white", width=r * 10
                )
    ...
    def get_img_agnostic(self, img, parse, pose_data):
        ...
        agnostic_draw.rectangle(
            (pointx - r * 7, pointy - r * 7, pointx + r * 7, pointy + r * 7),
            "gray",
            "gray",
        )
    ...
    def __getitem__(self, index):
        ...
        parse_agnostic_map = torch.zeros(
            20, self.load_height, self.load_width, dtype=torch.float
        )
        parse_agnostic_map.scatter_(0, parse_agnostic, 1.0)
        new_parse_agnostic_map = torch.zeros(
            self.semantic_nc, self.load_height, self.load_width, dtype=torch.float
        )
