...
class Vit_Lit(L.LightningModule):
    ...
    def _calculate_loss(self, batch, mode="train"):
        ...
        loss = F.mse_loss(
            coarse_result, target_img
        )  # + F.l1_loss(cloth_mask, target_cloth)
        ...
        if mode == "val":
            reconstructed_img_np = (
                torchvision.utils.make_grid(coarse_result, normalize=True)
                .cpu()
                .detach()
                .numpy()
            )
            original_img_np = (
                torchvision.utils.make_grid(target_img, normalize=True)
                .cpu()
                .detach()
                .numpy()
            )
            self.logger.experiment.add_image(
                f"{mode}_reconstructed_images",
                reconstructed_img_np,
